      ...
      <div class="column-navigation">
        ...
        <div class="scroll-indicator">
          <div 
            class="scroll-thumb"
            :style="{ 
              left: `${(activeColumnIndex / Math.max(1, columns.length - 1)) * 100}%`,
        ...
        >
          ...
          <div class="column-header">
            <div class="column-info">
              ...
              <span class="column-count">{{ getColumnCardCount(activeColumn) }} cards</span>
            </div>
            <button 
          ...
          <div class="cards-container">
            ...
            <!-- Empty State -->
            <div v-if="getColumnCards(activeColumn).length === 0" class="empty-column">
    ...
    <div v-if="showMoveModal" class="mobile-modal">
      ...
      <div class="modal-content">
        <div class="move-options">
          ...
          >
            <span class="column-name">{{ column.name }}</span>
            <span class="card-count">{{ getColumnCardCount(column) }} cards</span>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    getColumnCards(column) {
      return this.cards.filter(card => 
        card[`field_${this.view.kanban_field_id}`] === column.value
      )
    },
    ...
    handleColumnTouchMove(event) {
      ...
      if (Math.abs(deltaX) > 50) {
        if (deltaX > 0 && this.activeColumnIndex > 0) {
          this.previousColumn()
        } else if (deltaX < 0 && this.activeColumnIndex < this.columns.length - 1) {
          this.nextColumn()
        }
