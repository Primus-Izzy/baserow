...
export default {
  ...
  computed: {
    visibleFields() {
      return this.fields.filter(field => 
        ...
        this.isFieldInCurrentStep(field)
      )
    },
    ...
    formSteps() {
      ...
      this.fields.forEach(field => {
        if (field.step_break) {
          ...
          currentStep = { name: field.step_name || `Step ${steps.length + 1}`, fields: [] }
        }
    ...
    progressPercentage() {
      if (!this.isMultiStep) {
        const completedFields = this.fields.filter(field => 
    ...
    hasNextStep() {
      return this.isMultiStep && this.currentStepIndex < this.formSteps.length - 1
    },
  ...
  methods: {
    isFieldVisible(field) {
      ...
      return field.conditions.every(condition => {
        const conditionField = this.fields.find(f => f.id === condition.field_id)
        ...
        switch (field.type) {
          ...
          case 'number':
            if (isNaN(value)) {
              ...
              if (field.min_value !== undefined && parseFloat(value) < field.min_value) {
                errors.push(`Value must be at least ${field.min_value}`)
              }
              if (field.max_value !== undefined && parseFloat(value) > field.max_value) {
  ...
  mounted() {
    ...
    if (this.isMobileDevice) {
      this.$nextTick(() => {
        const firstInput = document.querySelector('.field-input-container input, .field-input-container textarea, .field-input-container select')
