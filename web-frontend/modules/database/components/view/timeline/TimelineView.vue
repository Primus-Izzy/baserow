        ...
        <div class="timeline-view__zoom-controls">
          <button
            ...
            :class="[
              ...
              { 'timeline-view__zoom-btn--active': currentZoom === zoomLevel.value }
            ]"
...
export default {
  ...
  computed: {
    ...
    visibleFields() {
      return this.fields.filter(
        (field) => !this.fieldOptions[field.id]?.hidden
      )
    },
    hiddenFields() {
      return this.fields.filter(
    ...
    visibleRows() {
      return this.allRows.filter((row) => {
        ...
        return rowStart <= this.visibleDateRange.end && 
               rowEnd >= this.visibleDateRange.start
      })
    },
    visibleDependencies() {
      return this.dependencies.filter((dep) => {
        const predecessorRow = this.allRows.find(r => r.id === dep.predecessor_row_id)
        const successorRow = this.allRows.find(r => r.id === dep.successor_row_id)
    ...
    activeSearchTerm() {
      return this.$store.getters[this.storePrefix + 'view/timeline/getActiveSearchTerm']
    },
  },
  mounted() {
  ...
  methods: {
    ...
    setupTouchGestures() {
      ...
      this.$refs.timelineContent.addEventListener('touchstart', handleTouchStart, { passive: false })
      this.$refs.timelineContent.addEventListener('touchmove', handleTouchMove, { passive: false })
    },
    ...
    zoomIn() {
      const currentIndex = this.zoomLevels.findIndex(z => z.value === this.currentZoom)
      if (currentIndex > 0) {
    ...
    getDaysInYear(date) {
      return ((date.getFullYear() % 4 === 0 && date.getFullYear() % 100 !== 0) || date.getFullYear() % 400 === 0) ? 366 : 365
    },
    ...
    async handleDateChange(row, field, newDate) {
      await this.updateValue(row, field, newDate, this.getRowDate(row, field.id))
        ...
        try {
          await this.$store.dispatch(
            this.storePrefix + 'view/timeline/recalculateSchedule',
            {
              viewId: this.view.id,
              rowId: row.id,
              newStartDate: field.id === this.view.start_date_field ? newDate : null,
              newEndDate: field.id === this.view.end_date_field ? newDate : null,
            }
          )
        } catch (error) {
    ...
    async handleDependencyCreated(predecessorRowId, successorRowId, dependencyType = 'finish_to_start') {
      try {
        await this.$store.dispatch(
